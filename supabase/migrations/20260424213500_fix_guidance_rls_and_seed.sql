-- ============================================================
-- Fix: Add RLS policies for guidance tables and seed data
-- ============================================================

-- Enable RLS (already enabled by default, but be explicit)
ALTER TABLE surgery_type_signs ENABLE ROW LEVEL SECURITY;
ALTER TABLE surgery_type_phase_guidelines ENABLE ROW LEVEL SECURITY;

-- Allow authenticated users to read signs and guidelines
CREATE POLICY "Allow authenticated read surgery_type_signs"
  ON surgery_type_signs FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Allow authenticated read surgery_type_phase_guidelines"
  ON surgery_type_phase_guidelines FOR SELECT
  TO authenticated
  USING (true);

-- ============================================================
-- Re-seed data (tables are empty due to RLS during initial seed)
-- ============================================================
